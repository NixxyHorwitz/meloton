...
<style>
.video-scroll { display: flex; gap: 12px; overflow-x: auto; padding-bottom: 8px; margin: 0 -16px; padding-left: 16px; padding-right: 16px; scroll-snap-type: x mandatory; scrollbar-width: none; }
.video-scroll::-webkit-scrollbar { display: none; }
.v-card { flex: 0 0 200px; scroll-snap-align: center; text-decoration: none; display: flex; flex-direction: column; background: var(--white); border: var(--border); border-radius: 12px; overflow: hidden; box-shadow: var(--shadow-sm); transition: all 0.2s; }
.v-card:hover { transform: translateY(-2px); box-shadow: var(--shadow); }
.v-card__thumb { position: relative; aspect-ratio: 16/9; background: #000; overflow: hidden; }
...
.v-card__badge { position: absolute; top: 6px; right: 6px; background: var(--brand); color: #fff; font-size: 9px; font-weight: 700; padding: 2px 6px; border-radius: 6px; }
...
.v-card__info { padding: 8px 10px; display: flex; flex-direction: column; gap: 4px; }
.v-card__title { font-size: 11px; font-weight: 700; color: var(--ink); display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; line-height: 1.3; height: 28px; }
.v-card__meta { display: flex; align-items: center; justify-content: space-between; font-size: 9px; font-weight: 600; color: var(--text-muted); }
</style>
...
<?php if (empty($videos)): ?>
<div class="card" style="margin-bottom:16px; border-color:#e2e8f0; border-radius:12px;">
...
<div class="card" style="margin-bottom:16px; border-radius:12px; overflow:hidden; border-color:#e2e8f0;">
